...
for cfg in "",default "--all-features --exclude gst-plugin-gtk4 --exclude gst-plugin-whisper --exclude gst-plugin-llamacpp",all "--no-default-features --exclude gst-plugin-burn --exclude gst-plugin-compress",no-default; do
    IFS="," read cfg junit <<< "${cfg}"

    echo "Building and testing with $cfg"

    cargo build $CARGO_FLAGS $build_cfg $cfg
    RUST_BACKTRACE=1 G_DEBUG=fatal_warnings cargo nextest run $CARGO_NEXTEST_FLAGS $CARGO_FLAGS $build_cfg $cfg
