...
pub fn register(plugin: &gst::Plugin) -> Result<(), glib::BoolError> {
    #[cfg(feature = "doc")]
    {
        use crate::raw_video::line_numbering::{
            LineNumberingIdentificationMethod, LineNumberingScheme,
        };

        ConcealmentMethod::static_type().mark_as_plugin_api(gst::PluginAPIFlags::empty());
        LineNumberingIdentificationMethod::static_type()
            .mark_as_plugin_api(gst::PluginAPIFlags::empty());
        LineNumberingScheme::static_type().mark_as_plugin_api(gst::PluginAPIFlags::empty());
    }
